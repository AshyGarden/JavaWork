...
public class EmployeeManagerFM {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(menu == 1) {
				...
				System.out.println("# 사원 정보 등록을 시작합니다."); //사번이외는 겹쳐도상관x
				System.out.println(" - 사번: ");
				String empNum = sc.next();
				while(true) { //중복횟수를 모르기때문에 무한 루프
					boolean flag = false; //중복 참거짓 변수
					for(int i=0; i<count; i++) { //등록된 사원까지만 체크(count)
					...
					if(!flag) { //입력한 사번이 중복이 발생하지 않은 경우 = 입력한 사번부터 넣기
						userNums[count] = empNum;
						break; //사원 중복확인용 무한루프 탈출
						
						
					} else { //flag == true? 입력한 사번이 중복이 발생한 경우
						System.out.print(" - 사번 재입력: ");
						empNum = sc.next(); // 기존에 변수가 있는경우 재사용 해도됨.
				...
				System.out.println(names[count] + "님의 정보가 정상 입력되었습니다."); 
				//
				count++; //다음사람은 다음 인덱스에 저장하기 위해
				...
				if(count == 0) {
					System.out.println("등록된 사운정보가 없습니다.");
				} else {
					System.out.println("==========전체사원 정보==========");
				...
				int i;
				for(i=0; i<count; i++) {
					if(empNum.equals(userNums[i])) { //입력받은 값이 사번과일치할때
				...
				//for문에 걸리지않음 = 사번이 존재하지않는경우
				if(i==count) {
					System.out.println("조회하시 사원정보는 존재하지않습니다.");
				}
...
//							System.out.print("> ");
//							ages[i] = sc.nextInt();
//							System.out.printf("나이가 %d세로 정산 변경되었습니다.\n",ages[i]);		
//							break;
//						case 2:
//							
//							break;
...
//							break;
//						default:
//								System.out.println(" - 변경 메뉴를 잘못입력하셨습니다.");
//						}
//						
//						flag = true;
//						break;
				...
				for(int i=0; i<count; i++) {
					if(empNum.equals(userNums[i])) {
						...
						case "Y": case "y": case "ㅛ":
							if(i==userNums.length-1) { //배열이 모두차서 마지막값일때 다읍값이 없기에 강제세팅
								userNums[i] = null;
								names[i] = null;
								ages[i] = 0; //int
								departments[i] = null;
							}
							for(int j=i; j<count-1; j++) { //배열의 마지막 값(j=99일때 )의 삭제때문에 특수처리필요
